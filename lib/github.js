...
const _getAllResultsByPage = (url, repos) => new Promise((resolve, reject) => http.get(url)
  .then(response => {
    ...
    const allTheReposCombined = repos ? reposWeJustFound.concat(repos) : reposWeJustFound;
    ...
    if (headers.link && ~headers.link.indexOf("next")) {
      const _nextPage = headers.link.split("<")[1].split(">")[0];
...
module.exports = {
  getGithubReposByOrg: orgs => _purgeCacheFile()
    .then(() => fs.readFile(config.repoCacheFile, 'utf8'))
    .then(fileContents => JSON.parse(fileContents))
    .catch(() => _getUserCredentials()
      .then(() => {
        const work = [];
        orgs.forEach(org => {
    ...
    .then(allRepos => {
      fs.writeFile(config.repoCacheFile, JSON.stringify(allRepos, null, 2), 'utf8');
      return allRepos;
    })
};
